...
object Binary {
  ...
  trait Projection extends RandomAccessSeq.Projection[Byte] with BinaryLike {
    ...
    private[immutable] def forcedSlice(from0: Int, until0: Int) =
...
trait Binary extends RandomAccessSeq[Byte] with Binary.BinaryLike with Serializable {
   ...
   */
  private[immutable] def depth: Int
   ...
   * result.
   */
  private[immutable] final def forcedSlice(from: Int, until: Int): Binary = {
   ...
   * result.
   */
  private[immutable] final def forcedSlice(from: Int) = slice(from, length)
